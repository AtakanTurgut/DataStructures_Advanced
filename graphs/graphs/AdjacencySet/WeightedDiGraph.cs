...
namespace graphs.AdjacencySet
{
        ...
        where TW : IComparable
    {
        ...
        public IEnumerable<IDiGraphVertex<T>> VerticesAsEnumberable => vertices.Select(x => x.Value);
        ...
        IEnumerable<IGraphVertex<T>> IGraph<T>.VerticesAsEnumberable => vertices.Select(x => x.Value);
        ...
        {
            var graph = new graphs.AdjacencySet.WeightedDiGraph<T, TW>();
        ...
        public IEnumerable<T> Edges(T key)
        {
            ...
            //return vertices[key].OutEdges.Select(e => e.Key.Key);
            return vertices[key].Edges.Select(e => e.TargetVertexKey);
        }
        ...
        public void RemoveVertex(T key)
        {
            ...
            if (!vertices.ContainsKey(key)) throw new ArithmeticException("The vertex is not in this graph.");
